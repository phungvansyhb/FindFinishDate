...
import { DocumentData, QueryConstraint, addDoc, collection, doc, getDoc, getDocs, limit, orderBy, query, setDoc, updateDoc, where, startAfter, DocumentSnapshot, writeBatch, runTransaction } from 'firebase/firestore';
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        for (const item of querySnapshot.docs) {
          ...
          const dataRef = doc(db, key+'/'+data.id)
          transaction.update(dataRef, { teacherAbsents: data.teacherAbsents + amount });
        }
      })
    } catch (e) {
      console.error("Error adding document: ", e);
